...
// console.log(haberContainer);

function haberYarat(f) {
  const haberYapici = document.createElement("div");
  haberYapici.setAttribute("class", "article");

  const title = document.createElement("h2");
  title.textContent = f.baslik;
  haberYapici.appendChild(title);

  const date = document.createElement("p");
  date.setAttribute("class", "tarih");
  date.textContent = f.tarih;
  haberYapici.appendChild(date);

  const first = document.createElement("p");
  first.textContent = f.ilkParagraf;
  haberYapici.appendChild(first);

  const sec = document.createElement("p");
  sec.textContent = f.ikinciParagraf;
  haberYapici.appendChild(sec);

  const third = document.createElement("p");
  third.textContent = f.ucuncuParagraf;
  haberYapici.appendChild(third);

  const myButton = document.createElement("span");
  myButton.setAttribute("class", "expandButton");
  myButton.textContent = "+";
  haberYapici.appendChild(myButton);
  myButton.addEventListener("click", (event) =>
    haberYapici.classList.toggle("article-open")
  );
...
data.forEach((item) => {
  haberContainer.appendChild(haberYarat(item));
});

// const yeniHaber1 = haberYarat(data);

// haberContainer.appendChild(yeniHaber1);

// haberContainer.appendChild(haberYapici);

// document.myButton.addEventListener("click"(myFunc(haberYapici)));

// function myFunc(event) {
//   event.setAttribute("class", "article-open");
// }
// console.log(haberYapici, title, txt1, txt2, txt3, myButton);
